...
function saveBlob(blob, name) {
  if (!blob || blob.size === 0) throw new Error(`No file data was generated for ${name}.`);
...
export default function App() {
  ...
  async function upload(selectedFile) {
    try {
      ...
      setMessage("Artwork uploaded. Choose bundles, then create files or ZIP package.");
    } catch (error) {
  ...
  function selectBundle(bundleKey) {
    ...
    setMessage(`${found?.label || "Bundle"} selected.`);
  }
  ...
  async function downloadSelectedFiles() {
    if (!url || selectedSizes.length === 0) {
      setMessage("Upload artwork and select at least one size.");
      return;
    }
    setBusy(true);
    try {
      for (let i = 0; i < selectedSizes.length; i += 1) {
        const size = selectedSizes[i];
        setMessage(`Creating ${i + 1} of ${selectedSizes.length}: ${size.label}`);
      ...
      setMessage("Selected files downloaded. Check your Downloads folder.");
    } catch (error) {
  ...
  async function createInstructions() {
    setBusy(true);
    try {
      setMessage("Creating buyer instruction sheet...");
      const blob = await createInstructionBlob();
      saveBlob(blob, `${slugify(title)}_customer_instructions.jpg`);
      setMessage("Customer instruction sheet downloaded.");
    } catch (error) {
      setMessage(error.message || "Instructions failed.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function generateMockup(mockup) {
    setBusy(true);
    try {
      setMessage(`Creating ${mockup.label}...`);
      const blob = await createMockupBlob(mockup);
      saveBlob(blob, `${slugify(title)}_${slugify(mockup.label)}.jpg`);
      setMessage(`${mockup.label} downloaded.`);
    } catch (error) {
      setMessage(error.message || "Mockup failed.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function bulkMockups() {
    ...
    try {
      for (let i = 0; i < MOCKUPS.length; i += 1) {
        ...
        setMessage(`Creating mockup ${i + 1} of ${MOCKUPS.length}: ${mockup.label}`);
        const blob = await createMockupBlob(mockup);
        saveBlob(blob, `${slugify(title)}_${slugify(mockup.label)}.jpg`);
        await new Promise((r) => setTimeout(r, 250));
      }
      setMessage("Bulk mockups downloaded. Check your Downloads folder.");
    } catch (error) {
      setMessage(error.message || "Bulk mockups failed.");
    } finally {
      setBusy(false);
    }
  }

  async function createListingAssetBlob(assetType) {
    if (!url) throw new Error("Upload an image first.");
    const img = await loadImage(url);
    const canvas = document.createElement("canvas");
    canvas.width = 2000;
    ...
    } else if (assetType === "how-it-works") {
      ctx.font = "700 82px Georgia, serif";
    ...
    } else if (assetType === "bundle-preview") {
  ...
  async function generateListingAsset(assetType) {
    setBusy(true);
    try {
      setMessage(`Creating ${titleCase(assetType)}...`);
      const blob = await createListingAssetBlob(assetType);
      saveBlob(blob, `${slugify(title)}_${assetType}_${slugify(niche)}_etsy-listing-photo.jpg`);
      setMessage(`${titleCase(assetType)} downloaded.`);
    } catch (error) {
      setMessage(error.message || "Listing image failed.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function createAllListingImages() {
    ...
    setBusy(true);
    const assets = ["hero", "size-guide", "how-it-works", "bundle-preview", "watermark"];
    try {
      for (let i = 0; i < assets.length; i += 1) {
        const asset = assets[i];
        setMessage(`Creating listing image ${i + 1} of ${assets.length}: ${titleCase(asset)}`);
        const blob = await createListingAssetBlob(asset);
        saveBlob(blob, `${slugify(title)}_${asset}_${slugify(niche)}_etsy-listing-photo.jpg`);
        await new Promise((r) => setTimeout(r, 250));
      }
      setMessage("All listing images downloaded. Check your Downloads folder.");
    } catch (error) {
      setMessage(error.message || "All listing images failed.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function downloadSeoText() {
    ...
    setMessage("SEO copy downloaded.");
  }

  async function copySeo() {
    try {
      await navigator.clipboard.writeText(seoText());
      setMessage("SEO copied to clipboard.");
    } catch {
      setMessage("Copy failed. You can manually copy the text from the SEO section.");
    }
  }
  ...
  async function createEtsyZip() {
    ...
    try {
      ...
      for (let i = 0; i < selectedSizes.length; i += 1) {
        const size = selectedSizes[i];
        setMessage(`ZIP: adding print ${i + 1} of ${selectedSizes.length}: ${size.label}`);
      ...
      for (let i = 0; i < assets.length; i += 1) {
        const asset = assets[i];
        setMessage(`ZIP: adding listing image ${i + 1} of ${assets.length}: ${titleCase(asset)}`);
        const blob = await createListingAssetBlob(asset);
  ...
  return (
    <main className="app-shell">
      <div className="dashboard">
        ...
        <div className="live-status"><strong>Status:</strong> {progress || status}</div>
          ...
          <div className="asset-panel">
            <Card className="section-card">
              ...
              <div className="bundle-grid">
                ...
                <button onClick={() => { setSelected(new Set(SIZE_PRESETS.map((s) => s.label))); setMessage("All sizes selected."); }}><strong>Select Everything</strong><span>Print, Etsy, and social sizes</span></button>
                <button onClick={() => { setSelected(new Set()); setMessage("All sizes cleared."); }}><strong>Clear All</strong><span>Start fresh</span></button>
              </div>
            </Card>
